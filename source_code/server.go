...
func APIHandler(w http.ResponseWriter, req *http.Request) {
	...
	case "/api/addClock": //添加闹钟
		...
		addClock(newClock)
		Log(fmt.Sprintf("新增闹钟,周期：%s，时间：%d-%d-%d，备注:%s",newClock.ClockCycle,newClock.Week,newClock.Hour,newClock.Minute,newClock.Remark))
	case "/api/addActivity": //添加活动
		ActivityStr := query.Get("ActivityStr")
		var newActivity Activity
		err = json.Unmarshal([]byte(ActivityStr), &newActivity)
		activityList = addActivity(newActivity, activityList, 0)
	...
	case "/api/assignClass": //新增课程
		ClassStr := query.Get("ClassStr")
		curriName := query.Get("NameStr")
		var newClass Schedule_Info
		err = json.Unmarshal([]byte(ClassStr), &newClass)
		assignClass(newClass, curriName)
	...
	case "/api/searchCurriculum": //搜索课程
		Text := query.Get("Text")
		optionStr := query.Get("Option")
		option, _ := strconv.Atoi(optionStr)
		result := searchCurriculum(Text, option)
	...
	case "/api/searchActivity": //搜索活动
		...
		optionStr := query.Get("Option")
		option, _ := strconv.Atoi(optionStr)
		result := searchActivity(Text, typeName, Content, option)
		data, _ := json.Marshal(result)
		w.Write(data)
	...
	case "/api/save":
		Log("正在压缩file目录...")
		Zip("document/file", "document/file.zip")
		Log("正在压缩log目录...")
		Zip("document/log", "document/log.zip")
		Log("数据保存成功！")
		WriteConfig()
	}
...
func StartServer() {
	...
	http.Handle("/", http.FileServer(http.Dir("source_code/GUI")))
	go ServerTest()
