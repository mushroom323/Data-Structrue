...
refreshSearchedSubjects = function(){
    ...
    var holder = $('#curriculumsHolder');
    var sortType = $('#curriculumModal input:radio:checked').val();
    var str = '';
    ...
    if(text == ""){
        ...
        $.get('/api/searchCurriculum', {Text:text, Option:sortType}, function(data){
...
assignClass = function(){
    ...
    var newClassGroup = [];
    var curriculumName = $('#assignClassModal .curriculumSelect').val();
    var classWeek = $('#assignClassModal .week').val();
    var classStartOrder = $('#assignClassModal .startTime').val();
    var classEndOrder = $('#assignClassModal .endTime').val();
    var classDistrict = $('#assignClassModal .district').val();
    var classSpot = $('#assignClassModal .spot').val();
    var classClassroom = $('#assignClassModal .classroom').val();
    
    var errorWarning = $('#assignClassModal .modal-footer span strong');

    var timeError = false;

    //看看这个时间段内有没有其他课程
    for(var i = classStartOrder; i <= classEndOrder; i++){
        $.each(curriculumSchedul, function(index_1, curriculumInSchedul){
            $.each(curriculumInSchedul.Info, function(index_2, classInSchedul){
                if((i == classInSchedul.Order) && (classWeek == classInSchedul.Week)){
                    timeError = timeError || true;
                }
            })
        })
    ...
    //看看时间段设置错没
    timeError = timeError || (classStartOrder > classEndOrder);
    ...
    if(timeError == true){
        ...
        for(var i = classStartOrder; i <= classEndOrder; i++){
            var newClass = {};
            newClass.Week = parseInt(classWeek);
            newClass.Order = parseInt(i);
            newClass.IsExam = false;
            newClass.District = classDistrict;
            newClass.Spot = classSpot;
            newClass.Classroom = classClassroom;
            newClassGroup.push(newClass);

            $.get("/api/assignClass", {NameStr:curriculumName, ClassStr:JSON.stringify(newClass)});////
        }
    
        $.each(newClassGroup, function(index, newClass){
            curriculumSchedul.find(function(value){
                return value.Name == curriculumName;
            }).Info.push(newClass);
        })
...
uploadHomework = function(){ //在这里进行ajax 文件上传 作业的信息
    ...
    //判断文件类型,我这里根据业务需求判断的是word/bmp文件
        ...
        errorWarning.html('请选择word、txt或bmp文件!');			
        return false;
    }
    else{
        errorWarning.html('');
